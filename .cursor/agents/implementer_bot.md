...
- The **gitlab-issue-dag-orchestration** orchestrator runs **`cd <worktree_path> && node scripts/bootstrap-worktree-env.mjs && rm -rf node_modules && rtk pnpm install --frozen-lockfile && rtk pnpm build`** right after **`rtk git worktree add`** (see Phase 4a in `/workspace/.cursor/skills/gitlab-issue-dag-orchestration/SKILL.md`). **`bootstrap-worktree-env.mjs`** pulls `apps/backend/.env`, `.env.mcp`, and `apps/backend/*.pem` from the primary checkout; then optional native deps (e.g. `@oxc-parser/binding-*`) link correctly.
