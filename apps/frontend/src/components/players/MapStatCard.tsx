        ...
        <div className="p-4">
          ...
          <div>
            ...
            {activeCategory === "positioning" && (
              <PositioningStats mapStat={mapStat} />
            )}
...
function CombatStats({ mapStat }: { mapStat: PlayerMapStats }) {
  ...
  const multiKillRounds =
    mapStat.multikill_2k +
    mapStat.multikill_3k +
    mapStat.multikill_4k +
    mapStat.multikill_5k;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2 text-center pt-2 border-t border-border">
        ...
        <div>
          <p className="text-lg font-bold">
            {mapStat.clutches_won}/{clutchTotal}
          </p>
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Tried {mapStat.trade_attempts} / {mapStat.trade_opportunities}{" "}
          opportunities
        </p>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {utilDmgPerRound.toFixed(1)} per round
        </p>
      </div>
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-bold">
            {mapStat.avg_enemy_flash_duration.toFixed(2)}s
          </span>
        </div>
        <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.min(100, mapStat.avg_enemy_flash_duration * 50)}%`
            }}
          />
        </div>
        <p className="text-xs text-muted-foreground mt-1">
          Enemy flash duration (avg)
        </p>
      </div>
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          ...
          <span
            className={`font-bold ${mapStat.avg_teammate_flash_duration <= 0.3 ? "text-green-400" : mapStat.avg_teammate_flash_duration <= 0.6 ? "text-amber-400" : "text-red-400"}`}
          >
        ...
        <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.max(0, 100 - mapStat.avg_teammate_flash_duration * 100)}%`
            }}
          />
        </div>
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-bold">
            {mapStat.crosshair_placement.toFixed(1)}°
          </span>
        </div>
        <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.max(0, 100 - mapStat.crosshair_placement * 5)}%`
            }}
          />
        </div>
        <p className="text-xs text-muted-foreground mt-1">
          Lower is better (deviation from head level)
        </p>
      </div>
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-bold">
            {mapStat.time_to_damage.toFixed(0)}ms
          </span>
        </div>
        <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.max(0, 100 - mapStat.time_to_damage / 10)}%`
            }}
          />
        </div>
        <p className="text-xs text-muted-foreground mt-1">Lower is better</p>
          ...
          {(() => {
            ...
            return (
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {tradeable} of {totalDeaths} deaths were in tradeable
                  positions
                </p>
              </div>
            );
                ...
                <>
                  <div className="h-2 bg-gray-800 rounded-full overflow-hidden flex">
                    {tradedT > 0 && (
                      <div
                        className="h-full bg-green-400/50"
                        style={{ width: `${(tradedT / totalT) * 100}%` }}
                      />
                    )}
                    {notTradedT > 0 && (
                      <div
                        className="h-full bg-amber-400/50"
                        style={{ width: `${(notTradedT / totalT) * 100}%` }}
                      />
                    )}
                    {isolatedT > 0 && (
                      <div
                        className="h-full bg-red-400/50"
                        style={{ width: `${(isolatedT / totalT) * 100}%` }}
                      />
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {tradedT}/{notTradedT}/{isolatedT}
                  </p>
                </>
              );
            })()}
                ...
                <>
                  <div className="h-2 bg-gray-800 rounded-full overflow-hidden flex">
                    {tradedCT > 0 && (
                      <div
                        className="h-full bg-green-400/50"
                        style={{ width: `${(tradedCT / totalCT) * 100}%` }}
                      />
                    )}
                    {notTradedCT > 0 && (
                      <div
                        className="h-full bg-amber-400/50"
                        style={{ width: `${(notTradedCT / totalCT) * 100}%` }}
                      />
                    )}
                    {isolatedCT > 0 && (
                      <div
                        className="h-full bg-red-400/50"
                        style={{ width: `${(isolatedCT / totalCT) * 100}%` }}
                      />
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {tradedCT}/{notTradedCT}/{isolatedCT}
                  </p>
                </>
              );
            })()}
