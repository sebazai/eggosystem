...
if [ "$WAIT_FOR_DB" = true ]; then
  ...
  until echo "SHOW DATABASES;" | mysql -h "$DB_HOST" -u"$DB_USER" -p"$DB_PASSWORD" | grep -w "$OLD_DB_NAME" > /dev/null; do
    sleep 5
  done
  ...
  until echo "SELECT COLUMN_NAME, EXTRA FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$OLD_DB_NAME' AND TABLE_NAME='trades' AND COLUMN_NAME='id' AND EXTRA LIKE '%auto_increment%';" \
    | mysql -h "$DB_HOST" -u"$DB_USER" -p"$DB_PASSWORD" "$OLD_DB_NAME" | grep -q "id"; do
