...
// Get organization channels and roles info
export const getOrganizationDiscordInfo = async (
  organizationName: string
): Promise<{
  roleId: string | null;
  channels: Array<{ gameType: string; channelId: string; channelName: string }>;
}> => {
  const client = await getDiscordClient();
  const guild = await client.guilds.fetch(process.env.DISCORD_GUILD_ID!);

  const role = guild.roles.cache.find(
    (r) => r.name.toLowerCase() === organizationName.toLowerCase()
  );
  ...
  const channels = guild.channels.cache
    .filter(
      (channel) =>
        channel.type === ChannelType.GuildText &&
        channel.name.toLowerCase().startsWith(organizationName.toLowerCase())
    )
    .map((channel) => {
      const gameType =
        Object.entries(GAME_CHANNEL_MAPPING).find(([_, channelSuffix]) =>
          channel.name.toLowerCase().includes(channelSuffix)
        )?.[0] || "unknown";
...
export const cleanupDiscordClient = async (): Promise<void> => {
  if (discordClient) {
    ...
    discordClient = null;
    eventHandlersSetup = false; // Reset flag so event handlers can be set up again
