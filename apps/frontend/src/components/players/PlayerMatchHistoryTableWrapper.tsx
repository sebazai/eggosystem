  ...
  const columns = useMemo<ColumnDef<MatchHistoryResult>[]>(
    () => [
      ...
      },
      {
        ...
        header: "",
        cell: ({ row }) => {
          const { match_id, match_game_id } = row.original;
          const href = match_game_id
            ? `/players/${steamId}/matches/${match_id}/games/${match_game_id}`
            : `/players/${steamId}/matches/${match_id}`;
          return (
            <Link
              href={href}
              ...
              title="Player stats for this game"
            >
              <svg
