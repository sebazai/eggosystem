...
import { leagueNameToTierKey } from "@/lib/matches/tiers";
import {
  stageLabel,
  stageKicker,
  durationLabel,
...
function buildTimeLabel(timestamp: string): string {
...
export function MatchCard({
  ...
  const tierKey = leagueNameToTierKey(match.league_name);
  const tierColor = `hsl(var(--tier-${tierKey}))`;
  ...
  const timeLabel = buildTimeLabel(match.start_timestamp);

  return (
    <Link href={href} className="block no-underline">
      {/* Desktop */}
      <div className="mb-2 hidden overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-colors hover:bg-accent/30 md:grid md:grid-cols-[6px_240px_1fr]">
        ...
        <div className="flex flex-col gap-1.5 border-r border-white/8 px-4 py-3">
          ...
          <div className="mt-1 flex flex-wrap gap-3">
            <MetaPill icon={Calendar} label={timeLabel} />
        ...
        <div className="flex flex-col justify-center px-4 py-3">
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
            ...
            <div className="flex items-center px-2 font-headings text-[2rem] leading-none">
              <span className={cn(!leftWins && rightWins && "opacity-40")}>
                {left.score}
              </span>
              <span className="mx-1 text-xl opacity-30">—</span>
              <span className={cn(!rightWins && leftWins && "opacity-40")}>
                {right.score}
              </span>
            </div>
      ...
      {/* Mobile */}
      <div
        className="mb-2 overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-colors hover:bg-accent/30 md:hidden"
        ...
        <div className="flex flex-col gap-2 px-3 py-3">
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
            ...
            >
              ...
              />
              <span className="font-headings text-sm leading-tight">
                {left.name}
              </span>
            </div>
            ...
            >
              <span className="font-headings text-right text-sm leading-tight">
                {right.name}
              </span>
              <NextImageFallback
                src={createTeamLogoUrl(right.logo ?? "")}
