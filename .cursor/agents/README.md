...
Specialized subagents for **gitlab-issue-dag-orchestration**. Each agent is invoked by the orchestrator via `Task(subagent_type=<name>)` and returns a strict JSON envelope (see `/workspace/.cursor/skills/json-handoff/SKILL.md`).
