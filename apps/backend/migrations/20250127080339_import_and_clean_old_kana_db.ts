...
export async function up(): Promise<void> {
  ...
  const rootDb = knex(rootDbConfig);

  try {
    console.log("Checking if 'kana' database exists...");
    // Check if the 'kana' database exists
    const [rows] = await rootDb.raw("SHOW DATABASES LIKE 'kana';");
    ...
    // Ensure permissions. This is necessary because the database might have been created manually
    await rootDb.raw("GRANT ALL PRIVILEGES ON kana.* TO 'kanadbuser'@'%';");
    await rootDb.raw("FLUSH PRIVILEGES;");

    console.log("Database 'kana' exists. Proceeding with migration.");
  } finally {
    await rootDb.destroy(); // Close temporary connection
  }
