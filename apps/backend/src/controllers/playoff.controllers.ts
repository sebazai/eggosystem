...
function isByeFaction(name: string, factionId: string): boolean {
  return name === BYE_NAME || !factionId || factionId.trim() === "";
}

function faceitStatusToMatchStatus(status: string): Match["status"] {
  const allowed: Match["status"][] = [
    "SCHEDULED",
    "CHECK_IN",
    "VOTING",
    "CONFIGURING",
  ...
  return allowed.includes(status as Match["status"])
    ? (status as Match["status"])
    : "SCHEDULED";
}

function scheduledAtToIso(scheduledAt: number | null | undefined): string {
  const ms =
 ...
 */
function getRound1SlotCount(maxSeed: number): number {
