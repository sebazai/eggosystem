...
export const getTeamPistolWinStatsController = async (
  req: RequestWithParams<{ team_id: string }>,
  res: Response
): Promise<void> => {
  const params = req.parsedParams;
  const pistolStats = await getTeamPistolWins(
...
export const getTeamPlantStatsController = async (
  ...
  const params = req.parsedParams;
  const plantStats = await getTeamPlantStats(
    Number(req.params.team_id),
    params
  );
...
export const getTeamRetakeStatsController = async (
  ...
  const params = req.parsedParams;
  const retakeStats = await getTeamRetakeStats(
    Number(req.params.team_id),
    params
  );
