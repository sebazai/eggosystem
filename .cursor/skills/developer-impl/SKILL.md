...
- A **canonical** repository path + branch name from the orchestrator (from `ops_bot`); the path should be absolute and match the **Worktree (absolute)** cell in `CONTEXT.local.md` (the primary clone: `cd "$(git rev-parse --show-toplevel)" && pwd -P`). The orchestrator should run `worktree_bot` / `pnpm run worktree:ensure` in that path first so pnpm and Husky resolve this tree’s `packages/`, not a symlinked `node_modules` from another clone. If you see `MODULE_NOT_FOUND`, knip, or linters flagging the wrong `packages/`, ask the PM to re-run the readiness step—do not symlink or copy `node_modules` from another checkout.
- A GitLab issue IID with acceptance criteria and the Explorer's `## Technical Brief`.
- **Local context file** — `CONTEXT.local.md` at the **repository root** of the handoff path (see [`.cursor/templates/CONTEXT.local.template.md`](../../templates/CONTEXT.local.template.md)). `ops_bot` writes a **stub**; you must ensure acceptance criteria, Technical Brief, and sub-issues are present (copy from the orchestrator’s first message if sections are still `[pending]` or empty).
