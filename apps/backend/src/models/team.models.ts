...
export const getTeamCaptainsBySeasonId = async (
  ...
  const results = await runQuery<TeamCaptain[]>(
    `SELECT 
      t.id AS team_id,
      t.name AS team_name,
      captain_discord.provider_username AS captain_discord,
      co_captain_discord.provider_username AS co_captain_discord
    FROM Teams t
    ...
    LEFT JOIN LinkedAccounts captain_discord ON 
      captain_discord.account_id = captain_account.id AND 
      captain_discord.provider = 'discord'
    ...
    LEFT JOIN LinkedAccounts co_captain_discord ON 
      co_captain_discord.account_id = co_captain_account.id AND 
      co_captain_discord.provider = 'discord'
    WHERE str.season_id = ?
    ORDER BY t.name`,
    [seasonId]
  );
