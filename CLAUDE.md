...
- **Interactive shells:** after changing `.devcontainer/`, **rebuild** the container so `remoteEnv` / `.bashrc` apply; or `export PATH="/usr/local/bin:/usr/local/share/npm-global/bin:/home/node/.local/bin:$PATH"` before `pnpm`.
...
- **Devcontainer image build (`npm error ECONNRESET`)**: transient registry/network failure during `docker build`. Retry **Rebuild Container**; the Dockerfile retries npm/corepack installs. If it persists, check VPN/proxy/Docker Desktop network settings.
