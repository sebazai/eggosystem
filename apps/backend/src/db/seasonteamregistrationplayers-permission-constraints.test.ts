...
describe("Captain Permission Triggers", () => {
  ...
  describe("SeasonTeamRegistrationPlayers Triggers (Registration Phase)", () => {
    it("adds ONLY permission (NO role) when captain is added during registration", async () => {
      await runQuery(
      ...
      // Should have permission but NOT role during registration
      expect(perms.length).toBeGreaterThan(0);
      expect(roles.length).toBe(0);
    });

    it("removes permission when captain status is revoked during registration", async () => {
      await runQuery(
        "INSERT INTO SeasonTeamRegistrationPlayers (season_id, team_id, steam_id, is_captain, is_co_captain) VALUES (999, 999, 76561198000000999, 1, 0)",
        []
      );
      ...
      // No role was granted, so nothing to check
      expect(roles.length).toBe(0);
    });
    ...
    it("deletes SeasonTeamRegistrationPlayers and captain permissions when SeasonTeamRegistration is deleted", async () => {
      // Setup: Add captain during registration
      await runQuery(
        "INSERT INTO SeasonTeamRegistrationPlayers (season_id, team_id, steam_id, is_captain, is_co_captain) VALUES (999, 999, 76561198000000999, 1, 0)",
        []
      );

      // Verify captain permissions exist (but NOT role - registration doesn't grant roles)
      ...
      // Verify SeasonTeamRegistrationPlayers are also deleted (cascade)
    ...
    it("transfers captain permissions when captain changes within same team during registration", async () => {
      ...
      // Add first captain during registration
      await runQuery(
        "INSERT INTO SeasonTeamRegistrationPlayers (season_id, team_id, steam_id, is_captain, is_co_captain) VALUES (999, 999, 76561198000000999, 1, 0)",
        []
      );

      // Verify first player has captain permissions (but NOT role during registration)
    ...
    it("transfers co-captain permissions when co-captain changes within same team during registration", async () => {
      // Setup: Add second player
      await runQuery(
        "INSERT INTO Accounts (id, work_email, full_name) VALUES (998, 'test2@example.com', 'Test User 2')",
        []
      ...
      // Add first co-captain during registration
      await runQuery(
      ...
      // Verify first player has co-captain permissions (but NOT role during registration)
      let perms1 = await runQuery<Array<AccountPermissionScopes>>(
        "SELECT * FROM AccountPermissionScopes WHERE account_id = 999",
        []
      );
    ...
    it("preserves captain permissions for other seasons/teams when captain changes in one team during registration", async () => {
      ...
      // Make player captain in both teams during registration
      await runQuery(
        "INSERT INTO SeasonTeamRegistrationPlayers (season_id, team_id, steam_id, is_captain, is_co_captain) VALUES (999, 999, 76561198000000999, 1, 0)",
        []
      );
      ...
      // Verify player has permissions for both teams (but NOT role during registration)
      let perms = await runQuery<Array<AccountPermissionScopes>>(
        "SELECT * FROM AccountPermissionScopes WHERE account_id = 999",
        []
      );
      expect(perms.length).toBeGreaterThan(0); // Should have 2 permissions (one per team)
      ...
      // Verify player still has permission for second team
      perms = await runQuery<Array<AccountPermissionScopes>>(
      ...
      // Verify player lost permission for first team
    ...
    it("allows captain to have both captain and co-captain permissions during registration", async () => {
      // Add player as both captain and co-captain during registration
      await runQuery(
      ...
      // Verify player has permissions (but NOT role during registration)
      const perms = await runQuery<Array<AccountPermissionScopes>>(
        "SELECT * FROM AccountPermissionScopes WHERE account_id = 999",
        []
      );
  ...
  }); // Close describe("SeasonTeamRegistrationPlayers Triggers (Registration Phase)")
}); // Close describe("Captain Permission Triggers")
