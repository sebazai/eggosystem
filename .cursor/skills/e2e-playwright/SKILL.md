...
2. Reseeds the E2E database (`pnpm --filter=backend reseed:e2e`)
3. Runs Playwright via `pnpm --filter=frontend test:e2e`
...
For a full run, `pnpm test:e2e` from root handles reseed and build; the backend must be reachable during the Playwright step (CI or a separate `dev:e2e` process).
