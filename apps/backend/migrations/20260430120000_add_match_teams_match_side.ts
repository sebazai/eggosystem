...
export async function up(knex: Knex): Promise<void> {
  ...
  // Round-robin BO2 stored as two Matches sharing external_match_room_id: first row (smaller id) aligns faction1→home / faction2→away; second row swaps sides.
  await knex.raw(`
    ...
    INNER JOIN (
      SELECT
        m_room.id AS match_id,
        COUNT(*) OVER (PARTITION BY m_room.external_match_room_id) AS matches_per_room,
        ROW_NUMBER() OVER (
          PARTITION BY m_room.external_match_room_id
          ORDER BY m_room.id ASC
        ) AS room_match_rn
      FROM Matches m_room
      WHERE m_room.season_id = 17
        AND m_room.external_match_room_id IS NOT NULL
        AND TRIM(m_room.external_match_room_id) != ''
    ) rm ON rm.match_id = m.id
    INNER JOIN (
      SELECT
    ...
    SET mt.match_side = CASE
      WHEN rm.matches_per_room = 2 AND rm.room_match_rn = 2 THEN CASE mt.team_id
        WHEN slt_home.team_id THEN 'away'
        WHEN slt_away.team_id THEN 'home'
      END
      ELSE CASE mt.team_id
