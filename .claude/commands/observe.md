...
argument-hint: <mr-iid> [grafana_url] [sentry_url]
---
...
2. `/workspace/.claude/agents/observer_bot.md` — the agent's spec.
...
Task(subagent_type=observer_bot,
     prompt="Read /workspace/.claude/agents/observer_bot.md. Analyze post-merge health for MR !<mr_iid> in project <group/project>. grafana_url=<grafana_url|null>. sentry_url=<sentry_url|null>. Return ONLY the JSON envelope.")
```
