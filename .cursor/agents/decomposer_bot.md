...
5. Optionally set **`implements_after_gates`** on a task (see `/workspace/.cursor/skills/json-handoff/SKILL.md`): for each **`p`** in **`depends_on`**, declare when **`pending` → Phase 4a** may begin. **Default omission** ⇒ **`completed`** parent (backward compatible).
