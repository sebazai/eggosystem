...
This repo runs a strict-JSON, DAG-driven pipeline as an alternative to the existing `/pm-plan` + `/pm-execute` flow. Both can coexist; pick per-issue.
...
- **`/pm-execute`** (existing) — single MR per issue, sub-specialists routed by `developer_bot`, prose Technical Brief. Best for small focused issues.
